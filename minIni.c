...
int ini_puts(const TCHAR *Section, const TCHAR *Key, const TCHAR *Value, const TCHAR *Filename)
{
  ...
  if (Key != NULL && Value != NULL) {
    ...
    /* Conversely, for a request to deleteOfflineMSG a setting; if that setting isn't
       present, just return */
    ...
    /* key found -> proceed to deleteOfflineMSG it */
  }
...
static void long2str(long value, TCHAR *str)
{
  ...
  } while (value /= 10);                /* deleteOfflineMSG the lowest digit */
  if (sign < 0)
    str[i++] = '-';
  str[i] = '\0';
